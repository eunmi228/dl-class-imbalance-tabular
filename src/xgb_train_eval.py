...
parser.add_argument('--datatype', type=str, choices=['both','control', 'rwdata'], default='control', help='dataset type: "both" or "control" or "rwdata". Default is "control".')
parser.add_argument('--device', type=int, help='cuda device number.')
import sys
# sys.argv = ['data_meta_features', '--datatype', 'rwdata', '--device', '4']
...
if args.datatype == 'both':
    datatype_list = ['control', 'rwdata']
else:
    datatype_list = [args.datatype]
class_imb_prep_list = [
            ClassImbPrep.NONE,
...
for datatype in datatype_list:
    ...
    for idx, elem in enumerate(iter_list):
        ...
        for class_imb_prep in class_imb_prep_list:
            ...
            train_dataset = ClassImbDataset(X_imb, Y_imb, device, class_imb_prep)
            ...
            model = XGBClassifier(gpu_id=gpu_id)
            if class_imb_prep==ClassImbPrep.CLASS_WEIGHT:
            ...
            optimizer = BayesianOptimization(f=xgb_hp_tune, pbounds=pbounds, random_state=random_state)
            optimizer.maximize(init_points=init_points, n_iter=n_iter, acquisition_function=acquisition_function)
            print(optimizer.max)
            best_params = optimizer.max['params']
            eta = best_params['eta']
            max_depth = int(best_params['max_depth'])
            reg_alpha = best_params['reg_alpha']
            row_eta[class_imb_prep] = eta
            row_max_depth[class_imb_prep] = max_depth
            row_reg_alpha[class_imb_prep] = reg_alpha
            f1_min_list = []
            f1_maj_list = []
            f1_macro_list = []
            for i in range(3):
        ...
        df_best_eta = pd.concat([df_best_eta, pd.DataFrame(row_eta, index=[0])], ignore_index=True)
        df_best_max_depth = pd.concat([df_best_max_depth, pd.DataFrame(row_max_depth, index=[0])], ignore_index=True)
        df_best_reg_alpha = pd.concat([df_best_reg_alpha, pd.DataFrame(row_reg_alpha, index=[0])], ignore_index=True)
